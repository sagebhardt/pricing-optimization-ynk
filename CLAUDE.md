...
- **action_type derives from PRICE direction, not step direction** (commit ad6eaeb). At channel grain, `channel_current_price` is a velocity-weighted median of observed prices and `channel_current_disc` is a velocity-weighted average of discount rates — they don't tie algebraically (`current_price ≠ list × (1 − current_disc)`). A step comparison would say "step 0.20 > current_disc 0.198 → decrease" while the actual prices go up (e.g., $79,990 → $89,990, a real recommendation we observed on SAMBA OG). Always derive `action_type` from `chosen_price` vs `current_price_rounded` so the classification matches what the BM sees on screen.
- **Urgency aggregation must align with action_type**: if `action_type == "decrease"`, filter `INCREASE` out of the per-store urgency pool before max-by-severity (otherwise `INCREASE` (severity 2) ties with `MEDIUM` and leaks through to channel rows that are actually markdowns, producing "Subir" badges on decrease rows).
